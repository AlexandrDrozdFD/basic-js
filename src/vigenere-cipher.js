...
class VigenereCipheringMachine {
  ...
  processEncryption(message, key, isEncrypt) {
    ...
    const encrypted = message
      ...
      .join('');
    return this.direction ? encrypted : this.reverseString(encrypted);
  }

  runFormula(isEncrypt, char, key, index) {
    if (!isEncrypt) {
      return (this.alphabet.indexOf(char) + 26 - this.alphabet.indexOf(key[index])) % 26;
    }
    return (this.alphabet.indexOf(char) + this.alphabet.indexOf(key[index])) % 26;
  }
